    ...
    html += `
      <div class="tree-group">
        <div class="tree-parent${isOpen ? ' open' : ''}${isActive ? ' active' : ''}"
          ...
          <span style="font-size:15px;">${cat.name === '肉类' ? '🥩' : cat.name === '米饭' ? '🍚' : cat.name === '面食' ? '🍜' : cat.name === '火锅' ? '🍲' : cat.name === '汤羹' ? '🥣' : cat.name === '家常菜' ? '🥬' : cat.name === '小吃' ? '🍢' : cat.name === '烘焙' ? '🍰' : cat.name === '饮品' ? '🥤' : cat.name === '甜品' ? '🍮' : '📖'}</span>
          <span>${cat.name}</span>
